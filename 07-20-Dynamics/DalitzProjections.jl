...
plot(layout=grid(1,3), size=(800,250),
...
plot(layout=grid(1,3), size=(800,250),	
	plot(σs->cosθ23(σs,ms^2), ms, c=cgrad(:roma, 10, categorical = true), aspect_ratio=1),
plot(σs->cosθ12(σs,ms^2), ms, c=cgrad(:roma, 10, categorical = true), aspect_ratio=1),
plot(σs->cosθ31(σs,ms^2), ms, c=cgrad(:roma, 10, categorical = true), aspect_ratio=1)
)
...
begin
	p = plot(layout=grid(2,3), size=(800, 500),	yaxis=nothing,
		map(1:3) do k
			i,j = ij_from_k(k)
	...
	plot!(layout=grid(2,1, heights=(0.01,0.99)), size=(800, 500/0.9),
		plot(title=model_name, xaxis=false, yaxis=false), p)
...
begin
	plot(layout=(@layout [A B{0.8w,0.8h} ; D C]),
		size=(700,700), link=:both)
	plot!(sp=1, 0:0.01:2, permute=(:x, :y), yaxis=nothing, xlab="m²(12)") do σk
				integrand = projection_integrand(I, ms, σk; k=1)
				quadgk(integrand, 0, 1)[1]
			end
	plot!(sp=2, I, ms, iσx=1, iσy=3,
			inset=(2,bbox(0.65,0.0,0.35,0.35)))
